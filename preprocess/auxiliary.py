...
def load_icd2name(doc_path: str, icd2code: Dict) -> Tuple[Dict, List]:
    """Matching ICD9CM codes with their names
    :param doc_path: Resource Path
    :param icd2code: code_map provided by EHR dataset
    :return icd2name: {ICD9CM code: ICD9CM LONG NAME}
    """
    icd9cm = pd.read_csv(os.path.join(doc_path, 'ICD9CM.csv'),
                         usecols=['code', 'name'])
    icd9cm = icd9cm.set_index('code').to_dict()['name']

    # Mapping ICD9CM codes to their names
    id2name = dict()
    for icd, _ in icd2code.items():
        name = icd9cm.get(icd, None)
        if name is None:  # Only for 719.70 -> 719.7
            name = icd9cm.get(icd[:-1], None)
        id2name[icd] = name

    # Double-checking unmatched icd codes
    unmatched = [icd for icd, name in id2name.items() if name is None]
    print('There is %d unmatched ICD9CM codes' % len(unmatched))

    return id2name, unmatched
...
if __name__ == '__main__':
    ...
    # Loading code_map for disease codes
    code_map = pickle.load(open(os.path.join(encoded_path, 'code_maps.pkl'), 'rb'))['code_map_pretrain']

    resource_path = '../resources'
    icd2name, _ = load_icd2name(resource_path, code_map)
    print('The length of icd2name:', len(icd2name))
