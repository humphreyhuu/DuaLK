...
class Mimic3Parser(EHRParser):
    ...
    def set_admission_col(self) -> dict:
        ...
        converter = {
            'SUBJECT_ID': int,
            'HADM_ID': int,
            'ADMITTIME': lambda cell: datetime.strptime(np.str_(cell), '%Y-%m-%d %H:%M:%S')
        }
        return converter
    ...
    def set_diagnosis_col(self) -> dict:
        ...
        converter = {'SUBJECT_ID': int, 'HADM_ID': int, 'ICD9_CODE': np.str_}
        return converter
...
def process_lab_events(folder_path: str, admission_codes: dict, dataset='mimic3') -> (dict, set):
...
def process_lab_items(folder_path: str, items_set: set, dataset='mimic3') -> dict:
