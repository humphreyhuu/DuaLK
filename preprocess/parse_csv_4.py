...
def process_lab_events(folder_path: str, admission_codes: dict, data='mimic3') -> (dict, set):
    if data == 'mimic3':
        lab_events = pd.read_csv("%s/LABEVENTS.csv.gz" % folder_path,
                                 usecols=['SUBJECT_ID', 'HADM_ID', 'ITEMID', 'FLAG'])
    elif data == 'mimic4':
        lab_events = pd.read_csv("%s/labevents.csv.gz" % folder_path,
                                 usecols=['subject_id', 'hadm_id', 'itemid', 'flag'])
        lab_events.columns = ['SUBJECT_ID', 'HADM_ID', 'ITEMID', 'FLAG']
    else:
        ValueError('data should be either mimic3 or mimic4')

    lab_events = lab_events.dropna(subset=['HADM_ID']).reset_index(drop=True)
    lab_events['HADM_ID'] = lab_events['HADM_ID'].astype(int)
    lab_events['FLAG'] = lab_events['FLAG'].map({'abnormal': 1, 'delta': 0}).fillna(0).astype(int)

    visit_labs = {k: [] for k in admission_codes.keys()}

    for hadm_id, group in lab_events.groupby('HADM_ID'):
        if hadm_id in admission_codes:
            visit_labs[hadm_id] = list(set(group[group['FLAG'] == 1]['ITEMID'].tolist()))

    items_set = set(lab_events[lab_events['HADM_ID'].isin(admission_codes.keys())]['ITEMID'])

    return visit_labs, items_set


def process_lab_items(folder_path: str, items_set: set, data='mimic3') -> dict:
    if data == 'mimic3':
        items = pd.read_csv("%s/D_LABITEMS.csv.gz" % folder_path,
                            usecols=['ITEMID', 'CATEGORY'])
    elif data == 'mimic4':
        items = pd.read_csv("%s/d_labitems.csv.gz" % folder_path,
                            usecols=['itemid', 'category'])
        items.columns = ['ITEMID', 'CATEGORY']
    else:
        ValueError('data should be either mimic3 or mimic4')

    items['CATEGORY'] = items['CATEGORY'].str.lower()
    items = items.set_index('ITEMID').to_dict()
    items_category = dict()

    for item_id in items_set:
        items_category[item_id] = items['CATEGORY'][item_id]
